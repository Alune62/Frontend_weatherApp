fetch('https://backend-weather-et59w0m3h-alune62.vercel.app/weather')
...
function updateDeleteCityEventListener() {
	for (let i = 0; i < document.querySelectorAll('.deleteCity').length; i++) {
		document.querySelectorAll('.deleteCity')[i].addEventListener('click', function () {
			fetch(`https://backend-weather-et59w0m3h-alune62.vercel.app/weather/${this.id}`, { method: 'DELETE' })
				.then(response => response.json())
				.then(data => {
					if (data.result) {
